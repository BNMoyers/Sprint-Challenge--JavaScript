...
const universities = [];
graduates.forEach((graduates,index) => {
...
let contactInfo = [];
function getContacts(graduate){
  return `${graduate.first_name} ${graduate.email}`
};
contactInfo = graduates.map(getContacts);
...
*/
const populationTotal = zooAnimals.reduce((runningTotal, individualPop) => {
  return runningTotal + individualPop.population;
}, 0);
